...
```typescript
// Upload file — takes File | Blob, NOT Buffer
...
// ALWAYS use the returned key and url — backend auto-renames on key conflict
// Save BOTH url and key to the database
...
- **No `upsert: true`** — this option does not exist in InsForge storage. The backend auto-renames files on key conflict (e.g. `resume.pdf` becomes `resume-<timestamp>.pdf`)
- Always use the **returned** `data.key` and `data.url` — never assume they match the path you provided
- Save **both** `url` AND `key` to the database — the key is required for future download/delete operations (Feature 07, 08)
- Upload takes `File | Blob` — never a Buffer. Buffer is Node.js only; use Blob in server contexts
- Never write files to disk — always upload directly to storage
...
// Upload directly to InsForge Storage — use Blob, not Buffer
const blob = new Blob([buffer], { type: 'application/pdf' });
const { data, error } = await insforge.storage
  .from('resumes')
  .upload(`resumes/${userId}/resume.pdf`, blob);
// Always use returned data.url and data.key
```
